...
echo "Mais ou est depose le script de startup ?"
echo "$0:a"
...
download_docker_image() {
  ...
  echo "Pulling latest image: gcr.io/cloud-datalab/datalab:latest"
  docker pull gcr.io/cloud-datalab/datalab:latest
...
clone_repo() {
  ...
  docker run --rm -v "${MOUNT_DIR}/content:/content"     --entrypoint "/bin/bash" gcr.io/cloud-datalab/datalab:latest     gcloud source repos clone datalab-notebooks /content/datalab/notebooks
}
...
populate_repo() {
  ...
  docker run --rm -v "${MOUNT_DIR}/content:/content"     --workdir=/content/datalab/notebooks     --entrypoint "/bin/bash" gcr.io/cloud-datalab/datalab:latest -c "        echo '.ipynb_checkpoints' >> .gitignore;         echo '*.pyc' >> .gitignore;         echo '# Project Notebooks' >> README.md;         git add .gitignore README.md;         git -c user.email=nobody -c user.name=Datalab           commit --message='Set up initial notebook repo.';         git push origin master;     "
}
